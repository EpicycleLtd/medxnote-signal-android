...
public class PushEditedTextJob extends PushSendJob implements InjectableType {

    private static final String TAG = PushEditedTextJob.class.getSimpleName();

    @Inject transient TextSecureCommunicationModule.TextSecureMessageSenderFactory messageSenderFactory;
    private long messageId;

    public PushEditedTextJob(Context context, long messageId, String destination) {
    ...
    protected void onSend(MasterSecret masterSecret) throws Exception {
        ...
        try {
            Log.w(TAG, "Sending edited message: " + messageId);
    ...
    @Override
    public void onAdded() {
        DatabaseFactory.getSmsDatabase(context).markAsEdit(messageId);
    }
